...
def finetune():
    config = load_config("configs/finetune_cls_scan.yaml")
    ...
    train_dataset = ScanObjectNNDataset(split="train", variant=train_cfg["variant"])
    test_dataset = ScanObjectNNDataset(split="test", variant=train_cfg["variant"])
    ...
    for epoch in range(train_cfg["epochs"]):
        ...
        if acc > best_acc:
            best_acc = acc
            no_improve = 0
            torch.save(model.state_dict(), os.path.join(output_dir, "checkpoints", "best_finetune_scan.pth"))
